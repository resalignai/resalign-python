...
from resalign import Resalign, AsyncResalign, DefaultAioHttpClient
from resalign._utils import is_dict
...
logging.getLogger("resalign").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[Resalign]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with Resalign(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncResalign]:
